...
def parseContainerResidual(split, base):
    global initialTime
    time = split[2]
    if(initialTime == -1):
        initialTime = long(time)
    ...
    for container in containers:
        if(len(container)>0):
            ...
            containerResidual[Id] += [float(value)]
...
with open(input_file) as f:
    ...
    for i in range(0, len(lines)):
        ...
        if (split[0] == 'DelayEstimateModel,' and split[4] == 'Arrival' and split[5] == 'Rate:'):
            parseContainerArrivalRate(split, base)
        if (split[0] == 'DelayEstimateModel,' and split[4] == 'Service' and split[5] == 'Rate:'):
            parseContainerServiceRate(split, base)
        if (split[0] == 'DelayEstimateModel,' and split[4] == 'Average' and split[5] == 'Delay:'):
            parseContainerWindowDelay(split, base)
        if (split[0] == 'DelayEstimateModel,' and split[4] == 'Longterm' and split[5] == 'Delay:'):
            parseContainerLongtermDelay(split, base)
        if (split[0] == 'DelayEstimateModel,' and split[4] == 'Residual:'):
            parseContainerResidual(split, base)

        #Add migration marker
        if (split[0] == 'Migration!'):
            i = split.index("time:")
            src = split[i+4]
            tgt = split[i+7].rstrip()
            time = split[i+1]
            time = (long(time) - initialTime)/base
        ...
        if (split[0] == 'Change' and split[1] == 'implemented'):
            i = split.index('from')
            src = split[i+1]
