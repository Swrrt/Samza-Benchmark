...
def parsePartitionArrived(split, base):
    ...
    for partition in partitions:
        if(len(partition)>0):
            ...
            partitionArrived[Id] += [long(value)]
            partitionArrivedT[Id] += [(long(time) - initialTime)/base]

def parsePartitionProcessed(split, base):
    ...
    for partition in partitions:
        if(len(partition)>0):
            ...
            partitionProcessed[Id] += [long(value)]
...
def parseContainerProcessed(split,  base, processed):
    global initialTime
    time = split[2]
    if(initialTime == -1):
        initialTime = long(time)
    ...
    for container in containers:
        if(len(container)>0):
            ...
            containerProcessed[Id] += [long(value)]
            containerProcessedT[Id] += [(long(time) - initialTime)/base]


def parseContainerDelay(split, base):
    global initialTime
    time = split[2]
    if(initialTime == -1):
        initialTime = long(time)
    ...
    for container in containers:
        if(len(container)>0):
            ...
            containerDelay[Id] += [float(value)]
            containerDelayT[Id] += [containerProcessed[Id][-1]]


def parseContainerFlushProcessed(split, base):
    global initialTime
    time = split[2]
    if(initialTime == -1):
        initialTime = long(time)
...
with open(input_file) as f:
    ...
    for i in range(0, len(lines)):
        ...
        if (split[0] == 'NetworkCalculusModel,' and split[4] == 'Arrived:'):
        ...
        if (split[0] == 'NetworkCalculusModel,' and split[4] == 'Processed:'):
            processed = {}
            parseContainerProcessed(split, base, processed)
            #Id = 'total'
            #if(Id not in containerArrived):
            #    containerProcessed[Id] = []
            #    containerProcessedT[Id] = []
        ...
        if (split[0] == 'NetworkCalculusModel,' and split[4] == 'Delay:'):
            parseContainerDelay(split, base)

        counter += 1
        if(counter % 100 == 0):
            print("Processed to line:" + str(counter))

        if (split[0] == 'NetworkCalculusModel,' and split[4] == 'Partition' and split[5] == "Processed:"):
            split = line.replace('Partition ', '').split(' ')
            parsePartitionProcessed(split, base)

        if (split[0] == 'NetworkCalculusModel,' and split[4] == 'Partition' and split[5] == "Arrived:"):
            split = line.replace('Partition ', '').split(' ')
...
#     xx = np.arange(0., 40000., 10)
#     img = plt.plot(xx , xx,'r-')
#     ii = 0
...
for Id in containerDelay:
    ...
    fig = plt.figure(figsize=(25,15))
    plt.plot(containerDelayT[Id], containerDelay[Id],'bo')#, containerRealDelayT[Id], containerRealDelay[Id], 'r^')
    plt.xlabel('X-th Message')
    plt.ylabel('Delay (ms)')
    plt.xticks(range(-100,600,10))
    axes = plt.gca()
    ...
    #plt.show()
    plt.grid(True)
    print("Generate figure for container " + Id + "'s delay")
    plt.savefig('figures/Container_'+Id+'_Delays.png')
    plt.close(fig)
    print(containerDelay[Id])
...
for Id in containerProcessed:
    ...
    #plt.yticks(range(0,000.0,500.0))
    #axes = plt.gca()
    #axes.set_ylim([0,1000])
    #axes.set_xlim([15000,30000])
    #plt.show()
    plt.grid(True)
   # print(containerArrived[Id])
   #print(containerProcessed[Id])
    print("Generate figure for container " + Id + "'s Arrived and Processed")
...
for Id in partitionProcessed:
    ...
    #plt.show()
    plt.grid(True)
    if (not os.path.isdir('figures/partitions')):
        print("Making directory")
        os.mkdir('figures/partitions/')
    print("Generate figure for partition " + Id + "'s Arrived and Processed")
